...
inline [[nodiscard]] juce::String abNextSideButtonText(bool next_is_a) {
    return juce::String::fromUTF8(
        next_is_a ? "\xE2\x96\xB6" "A" : "\xE2\x96\xB6" "B");
}

inline [[nodiscard]] bool sameEditableTimbreLibraryEntry(
...
inline [[nodiscard]] bool isCommandLetter(
...
inline [[nodiscard]] bool componentWindowIsActive(
...
inline [[nodiscard]] bool physicalKeyIsDown(std::uint16_t scan_code) {
...
inline [[nodiscard]] bool virtualKeyIsDown(int virtual_key) {
    return virtual_key != 0
...
inline [[nodiscard]] mgstc::engine::WavePcm makeSccReferencePcm(
...
inline [[nodiscard]] mgstc::engine::WavePcm makeOpllReferencePcm(
...
inline [[nodiscard]] juce::File pendingConversionFile(
    const juce::String& target) {
    return mgstcApplicationDataDirectory().getChildFile(
        "pending-" + target + "-conversion.txt");
}


inline void configureSettingsButton(
...
inline [[nodiscard]] AnimatedGifSupport& ensureGdiplusInitialized() {
    static AnimatedGifSupport runtime;
    return runtime;
}

inline [[nodiscard]] juce::Image gdiplusBitmapToImage(
...
inline [[nodiscard]] juce::Image loadImageMemory(
    const void* data,
    std::size_t size) {
...
inline [[nodiscard]] juce::Image loadImageFileForBackground(
...
inline [[nodiscard]] juce::Image loadImageBytesForBackground(
    const std::vector<std::uint8_t>& bytes) {
    return loadImageMemory(bytes.data(), bytes.size());
}

inline bool loadAnimatedGifFrames(
    const void* data,
    int size,
...
[[nodiscard]] inline mgstc::engine::OpllApproximationOptions
...
inline void configureImmediateAuditionButton(
    juce::DrawableButton& button,
    bool enabled,
    std::function<void()> action) {
    const auto normal = makeEditorIcon(
...
inline void configureMasterVolumeSlider(
    juce::Slider& slider,
    ...
    slider.setValue(
        session.snapshot().master_volume_percent,
        juce::dontSendNotification);
...
inline void configureMasterVolumeLabel(juce::Label& label) {
    label.setText(
...
inline void synchronizeMasterVolumeSlider(
    juce::Slider& slider,
    std::uint64_t& observed_revision,
    EditorSession& session) {
    const auto snap = session.snapshot();
    if (snap.master_volume_revision == observed_revision) {
        return;
    }
    observed_revision = snap.master_volume_revision;
    slider.setValue(
...
inline void layoutTimbreLibraryPanel(
...
inline void layoutCompositeLibraryPanel(
    juce::Rectangle<int> panel,
...
inline void layoutCompositeLayerLibraryPanel(
    juce::Rectangle<int> panel,
    juce::Label& title,
    juce::Label& hint,
    juce::ToggleButton& favorite_only) {
...
inline void assignExplicitFocusOrders(
...
inline [[nodiscard]] SnapshotResult writeComponentPngSnapshot(
...
public:
    ...
    explicit PerformanceKeyboard(EditorSession& session)
        : session_(session),
          keyboard_(
              keyboard_state_,
        ...
        pc_octave_ = standaloneHostPreferences(
        ...
        if (session_.snapshot().capabilities.midi_device_settings) {
            syncMidiControls();
            session_.midi().addDrainListener(this);
        } else {
            greyOutHostControl(midi_input_);
            greyOutHostControl(midi_status_);
        }
    ...
    ~PerformanceKeyboard() override {
        stopTimer();
        if (session_.snapshot().capabilities.midi_device_settings) {
            session_.midi().removeDrainListener(this);
        }
...
private:
    ...
    [[nodiscard]] SnapshotResult showAppSettings(
        ...
        public:
            SettingsContent(
                ...
                for (const auto& device : session_.snapshot().midi_devices) {
                ...
                pc_audio_kind_.setSelectedId(
                    session_.snapshot().pc_audio_backend
                            == PcAudioBackend::Asio
                        ? 2
                        : 1,
                    juce::dontSendNotification);
                ...
                const auto saved_driver =
                    session_.snapshot().asio_driver_name;
                int saved_driver_id{};
                int driver_id = 1;
                if (session_.snapshot().capabilities.audio_device_settings) {
                    for (const auto& driver :
                         session_.output().availableAsioDrivers()) {
                ...
                output_kind_.setSelectedId(
                    session_.snapshot().hardware_output
                        ? 2
                        : 1,
                    juce::dontSendNotification);
                ...
                const auto mamidi =
                    session_.snapshot().hardware_settings;
                ...
                opll_silence_seconds_.setText(
                    juce::String(
                        session_.snapshot().opll_keyoff_silence_seconds),
                    false);
                ...
                output_status_.setColour(
                    juce::Label::textColourId,
                    juce::Colour(0xFF9AA8B5));
                const auto output_caps = session_.snapshot().capabilities;
                output_status_.setText(
                    (output_caps.hardware_output || output_caps.mamidi)
                        ? juce::String::fromUTF8(
                              session_.output().hardwareStatus().c_str())
                        : juce::String{},
                    juce::dontSendNotification);
            ...
            void refreshOutputStatus() {
                ...
                output_status_.setText(
                    (caps.hardware_output || caps.mamidi)
                        ? juce::String::fromUTF8(
                              session_.output().hardwareStatus().c_str())
                        : juce::String{},
                    juce::dontSendNotification);
            ...
            void refreshPcAudioStatus() {
                pc_audio_status_.setText(
                    juce::String::fromUTF8(
                        session_.snapshot().pc_audio_status.c_str()),
                    juce::dontSendNotification);
                if (!session_.snapshot().capabilities.audio_device_settings) {
                    asio_settings_.setEnabled(false);
                    return;
                }
                asio_settings_.setEnabled(
                    session_.snapshot().asio_control_panel_available);
            }

            void applyCapabilityGrey() {
                const auto caps = session_.snapshot().capabilities;
                if (!caps.midi_device_settings) {
                    greyOutHostControl(midi_input_);
                    greyOutHostControl(apply_midi_);
                    greyOutHostControl(refresh_);
                }
                if (!caps.audio_device_settings) {
                    greyOutHostControl(pc_audio_kind_);
                    greyOutHostControl(asio_driver_);
                    greyOutHostControl(asio_settings_);
                }
                if (!caps.hardware_output && !caps.mamidi && !caps.vsif) {
                    greyOutHostControl(output_kind_);
                    greyOutHostControl(host_);
                    greyOutHostControl(port_);
                    greyOutHostControl(unit_);
                    greyOutHostControl(scc_plus_);
                    greyOutHostControl(waveform_monitor_);
                    greyOutHostControl(opll_silence_seconds_);
                    greyOutHostControl(apply_output_);
                    greyOutHostControl(reconnect_);
                }
            }
        ...
        content->ui_scale_.onChange = [safe, safe_content, safe_dialog] {
            ...
            UiScale::setGlobalPercent(
                percent,
                standaloneHostPreferences(
                    safe->session_.snapshot().capabilities));
        ...
        content->refresh_.onClick =
            [safe, safe_dialog] {
                if (safe == nullptr
                    || !safe->session_.snapshot()
                            .capabilities.midi_device_settings) {
                    return;
                }
                safe->session_.midi().refreshDevices(false);
                safe->syncMidiControls();
                if (safe_dialog != nullptr) {
        ...
        content->apply_output_.onClick = [safe, safe_content] {
            ...
            const auto caps = safe->session_.snapshot().capabilities;
            if (!caps.audio_device_settings
                && !caps.hardware_output
                && !caps.mamidi
                && !caps.vsif) {
                return;
            }
            ...
            if (!safe->session_.output().applyPcAudio(
                    pc_backend,
                    driver_name)) {
            ...
            const int silence_seconds = juce::jlimit(
                0,
                9999,
            ...
            if (!safe->session_.output().applySoundRoute(
                    hardware, std::move(settings), silence_seconds)
                && hardware) {
        ...
        content->asio_settings_.onClick = [safe, safe_content] {
            ...
            static_cast<void>(
                safe->session_.output().showAsioControlPanel());
            if (safe->session_.snapshot().pc_audio_backend
                != PcAudioBackend::Asio) {
                safe_content->pc_audio_kind_.setSelectedId(
                    1, juce::sendNotificationSync);
            }
        ...
        content->reconnect_.onClick = [safe, safe_content] {
            ...
            static_cast<void>(
                safe->session_.output().reconnectHardware());
            safe_content->refreshOutputStatus();
            safe->syncMidiControls();
        };
    ...
    void syncMidiControls() {
        ...
        if (!snap.capabilities.midi_device_settings) {
            midi_status_.setText({}, juce::dontSendNotification);
            return;
        }
        midi_service_revision_ = snap.midi_revision;
        midi_input_.clear(juce::dontSendNotification);
        midi_input_.addItem(
            juce::String::fromUTF8("MIDI入力なし"), 1);
        for (int index = 0;
             index < static_cast<int>(snap.midi_devices.size());
             ++index) {
            midi_input_.addItem(
                snap.midi_devices[static_cast<std::size_t>(index)].name,
                index + 2);
        }
        syncing_midi_controls_ = true;
        midi_input_.setSelectedId(
            snap.midi_selected_combo_id,
            juce::dontSendNotification);
        syncing_midi_controls_ = false;
        auto status = snap.midi_status;
        if (snap.capabilities.hardware_output || snap.capabilities.mamidi) {
            status += " | ";
            status += juce::String::fromUTF8(
                session_.output().hardwareStatus().c_str());
        }
        midi_status_.setText(status, juce::dontSendNotification);
    }
    ...
    void drainPendingMidi() {
        for (const auto& message :
             session_.midi().takePendingMessages()) {
    ...
    void serviceActiveWindowInput(bool poll_pc_keys) {
        ...
        const auto caps = session_.snapshot().capabilities;
        if (caps.midi_device_settings
            && midi_service_revision_ != session_.snapshot().midi_revision) {
            syncMidiControls();
        }
        if (!windowIsActive()) {
            allNotesOff();
        ...
        if (caps.midi_device_settings) {
            if (!was_window_active_) {
                session_.midi().clearPendingMessages();
...
public:
    ...
    LibraryManagerContent(
            ...
            UiLayout::libraryManagerH);
        {
            ...
            ui_scale_listener_ = UiScale::addGlobalListener([safe] {
                if (safe == nullptr) {
                    return;
                }
                UiScale::forceGlobalForNonEditorUi();
...
private:
    ...
    std::uint64_t ui_scale_listener_{};
};
...
public:
    explicit CompositeEditorComponent(
        ...
        std::optional<mgstc::engine::CompositeTimbre> initial_timbre = {})
        : open_editor_(std::move(open_editor)),
        ...
        configureMasterVolumeSlider(
            master_volume_, session_, this);
        master_volume_revision_ =
            session_.snapshot().master_volume_revision;
        ...
        tag_manage_.onClick = [this] {
            if (!session_.snapshot().capabilities.tone_library) {
                return;
            }
            if (manage_tags_) {
                manage_tags_(this);
        ...
        timeline_.setEditorSession(&session_);
        performance_keyboard_.setCallbacks(
            [this](std::uint8_t note) {
    ...
    void auditionImportedCandidate(
        ...
        composite_program_stale_ = true;
        if (!session_.snapshot().audio_running) {
            engine_ready_ = false;
...
private:
    ...
    void prepareEngineForProgramEdit() {
        session_.audition().flushPending();
    }

    void applyHostCapabilities() {
        const auto caps = session_.snapshot().capabilities;
        if (!caps.spectrum_analyzer && !caps.spectrogram) {
        ...
        if (!caps.tone_library) {
            ...
            greyOutHostControl(layer_library_hint_);
            greyOutHostControl(composite_filter_);
            greyOutHostControl(composite_tag_filter_);
            greyOutHostControl(tag_manage_);
            greyOutHostControl(composite_favorite_only_);
            greyOutHostControl(composite_sort_);
            greyOutHostControl(layer_favorite_only_);
            greyOutHostControl(composite_select_);
            greyOutHostControl(new_);
            greyOutHostControl(load_);
            greyOutHostControl(save_);
            greyOutHostControl(save_as_);
            greyOutHostControl(composite_ab_);
            greyOutHostControl(duplicate_);
            greyOutHostControl(rename_);
            greyOutHostControl(delete_);
        }
    }
    ...
    void silenceAuditionNotes() {
        ...
        for (const auto track : sounding_tracks_) {
            static_cast<void>(
                session_.audition().noteOff(track));
            if (track >= kOpllTrack) {
    ...
    void startLayerNote(std::uint8_t track, std::uint8_t note) {
        if (track >= kOpllTrack) {
            session_.audition().cancelOpllKeyOffSilence(track);
        }
        if (session_.audition().noteOn(track, note)) {
    ...
    void timerCallback() override {
        ...
        const auto owned_rev = link_.owned_revision;
...
public:
    ...
    [[nodiscard]] bool needsSharedEngineReconfigure() const {
        ...
        if (!session_.snapshot().shared_program_active) {
            return true;
        }
        if (scc_wave_ != session_.snapshot().shared_scc_waveform) {
            return true;
        }
        return applied_shared_scc_revision_
                != session_.snapshot().shared_scc_revision
            || applied_shared_opll_revision_
                != session_.snapshot().shared_opll_revision;
    }
    ...
    [[nodiscard]] bool tryLoadOwnedSnapshot(std::uint64_t id) {
        const auto& target = link_.owned_target;
        if (!target
    ...
    void publishOwnedSnapshot() {
        ...
        auto snapshot = link_.owned_target;
...
private:
    ...
    void clearEngineVoices() {
        for (std::uint8_t channel = 0; channel < 5; ++channel) {
            static_cast<void>(session_.audition().noteOff(static_cast<std::uint8_t>(
                        kSccTrack + channel)));
        }
        static_cast<void>(voice_allocator_.allNotesOff());
    }
    ...
    void silenceAllVoices() {
        ...
        if (usesCompositeAudition()) {
            link_.owned_audition.stop_preview();
        }
        clearEngineVoices();
    }
    ...
    void applyHostCapabilities() {
        ...
        if (!caps.tone_library) {
            ...
            greyOutHostControl(tag_manage_);
            greyOutHostControl(favorite_only_);
            greyOutHostControl(library_ab_);
            greyOutHostControl(library_sort_);
            greyOutHostControl(library_list_);
            greyOutHostControl(library_load_);
            greyOutHostControl(library_delete_);
            greyOutHostControl(library_rename_);
        }
    }
    ...
    bool configureEngine(
        ...
        if (usesCompositeAudition()) {
            ...
            publishOwnedSnapshot();
            link_.owned_audition.sync_program();
            return true;
        }
        MGSTC_UI_ACTIVITY("scc: configureEngine (shared program)");
        clearEngineVoices();
        engine_holds_temporary_program_ = (preview != nullptr);
        const auto& waveform = preview != nullptr ? *preview : scc_wave_;
        const auto shared = session_.snapshot();
        SharedAuditionRequest request;
        request.scc = &waveform;
        request.opll = &shared.shared_opll_patch;
        request.retrigger = retrigger;
        request.track = kSccTrack;
        request.note = last_audition_note_;
        request.commit_scc = preview == nullptr;
        const auto configured = session_.audition().submitShared(request);
        if (configured && preview == nullptr) {
            const auto applied = session_.snapshot();
            applied_shared_scc_revision_ = applied.shared_scc_revision;
            applied_shared_opll_revision_ = applied.shared_opll_revision;
        }
        return configured;
    }
    ...
    void startPerformanceNote(std::uint8_t note) {
        ...
        if (usesCompositeAudition()) {
            publishOwnedSnapshot();
            link_.owned_audition.note_on(note);
            return;
        }
        if (!engine_ready_) {
            return;
    ...
    void stopPerformanceNote(std::uint8_t note) {
        audition_stop_time_ms_.reset();
        if (usesCompositeAudition()) {
            link_.owned_audition.note_off(note);
            return;
        }
        ...
        static_cast<void>(session_.audition().noteOff(static_cast<std::uint8_t>(kSccTrack + *channel)));
    }
    ...
    [[nodiscard]] bool auditionOneSecond(
        ...
        if (usesCompositeAudition()) {
            publishOwnedSnapshot();
            link_.owned_audition.one_second();
            armOneSecondPreview();
            return true;
        }
...
public:
    ...
    [[nodiscard]] bool needsSharedEngineReconfigure() const {
        ...
        if (patch_ != session_.snapshot().shared_opll_patch) {
            return true;
        }
        return applied_shared_scc_revision_
                != session_.snapshot().shared_scc_revision
...
private:
    ...
    void convertToScc(int frame) {
        session_.audition().publishSharedOpll(patch_);
        static_cast<void>(configureEngine(false));
        updateStatus(juce::String::fromUTF8(
    ...
    void applyHostCapabilities() {
        ...
        if (!caps.tone_library) {
            ...
            greyOutHostControl(library_list_);
            greyOutHostControl(library_load_);
            greyOutHostControl(library_save_);
            greyOutHostControl(library_save_as_);
            greyOutHostControl(library_rename_);
            greyOutHostControl(library_delete_);
            greyOutHostControl(library_import_);
            greyOutHostControl(library_export_);
        }
    }
    ...
    bool configureEngine(
        ...
        engine_holds_temporary_program_ = (preview != nullptr);
        const auto& patch = preview != nullptr ? *preview : patch_;
        const auto shared = session_.snapshot();
        SharedAuditionRequest request;
        ...
        request.note = last_audition_note_;
        request.commit_opll = preview == nullptr;
        const auto configured = session_.audition().submitShared(request);
        if (configured && preview == nullptr) {
            const auto applied = session_.snapshot();
            applied_shared_scc_revision_ = applied.shared_scc_revision;
    ...
    void timerCallback() override {
        ...
        if (session_.snapshot().capabilities.waveform_scope
            && componentWindowIsActive(*this)
            && session_.pollLatestOpllScope(frame)) {
