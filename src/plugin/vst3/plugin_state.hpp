...
// tone_library.sqlite. v2 adds Master Volume (0-100). v1 blobs still parse
// and restore Master Volume as 100. Unknown schema versions are not read.
inline constexpr std::uint32_t kPluginStateSchemaVersion = 2;
inline constexpr std::uint32_t kPluginStateSchemaVersionV1 = 1;
...
// master_volume_percent is persistent Processor gain (not Cubase fader).
