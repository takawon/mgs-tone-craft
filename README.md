...
- **総合のエンベロープ編集（開発中）**: 共通時間軸での音量／音程／休符など。`@e` プレビューあり（一部操作は未完成）
...
総合画面の複合エンベロープは Alpha で大きく進んでいますが、完了機能としては未保証です
（カウント命令の完全な直列化や曲用の本格的な`.MUS`出力など、画面上にあっても約束しません）。

音色ライブラリは**現状でも保存・読込はできます**が、上記 Alpha 方針のとおり版間の後方互換はありません。
現行形式より古いライブラリファイルは読み込めません。設定 INI や複合音色の保存についても同様です。
