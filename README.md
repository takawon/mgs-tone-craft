...
- **SCC単音色**: 32サンプル波形の編集、プリセット／マージ、ライブラリ保存、`@s`定義の確認・入出力
- **OPLL単音色**: MOD／CARパラメーター編集、ライブラリ保存、`@v`定義の確認・入出力
...
音色ライブラリは**現状でも保存・読込はできます**が、機能としては未完成です。
形式・操作・保存先の扱いが大きく変わる可能性があり、Alpha時点のデータを将来版で
そのまま使えることは保証しません。大切な音色は MGSC 定義テキスト（`.txt`）や
...
- ライブラリ操作は右側パネルの `新規`／`読込`／`保存`／`別名保存`／`取消`／`削除`／`取込`／`書出` です
...
- **Outputタブ**: 試聴出力先（内蔵エミュレータ／MAmi-VSIF dongle・MAmidiMEmo 経由の実機など）
- **Aboutタブ**: アプリ名・バージョンなどの表示
...
不具合や要望は GitHub の Issue へお願いします。
