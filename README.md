...
音色ライブラリは**現状でも保存・読込はできます**（インポートや管理窓の追加後も同様です）。
ただし現状は Alpha のため、**保存形式は今後変わり、新旧で読めなくなる可能性があります。**
設定 INI・単音色／複合ライブラリ・`.mgstc` の版間アップグレード互換は保証しません。
互換の維持は Beta 以降で開始します。大切な音色は MGSC 定義テキスト（`.txt`）や
...
| 種類 | 用途 |
|---|---|
| 音色ライブラリ（アプリ内） | 名前・タグ・メモ・お気に入り付きで SCC／OPLL／総合音色を保存・検索・読込 |
| `.mgstc` | ライブラリ項目の外部取込／書出（編集用。1件単位のやり取り向け） |
...
- **ライブラリは Alpha のままです。** 管理窓やインポートなど機能は増えていますが、保存形式は今後変わり、以前のライブラリ／`.mgstc` が読めなくなる可能性があります。版間の移行は保証しません（Beta 以降で互換方針を開始）
