...
発音中のPSG／SCC／OPLLを音源別に色分けして表示します。時間・周波数のズーム、周波数スクロール、現在の発音音程に基づく倍音ガイド、表示のみを強める0～100%の強調率を備えます。音源の表示ON／OFFや強調率の変更ではFFTを再計算しません。
...
- 総合のエンベロープ編集: 共通時間軸での音量／音程／休符など。音量は 0～15 等間隔で編集でき、チップ固有の論理カーブを重ねて表示します。音量ステップの「自動変化」は開始～終端のライン棒と MGSC `f=<n>`／`2n cc` ランプへ変換され、実効バーはドライバと同じステップで減衰します
...
総合画面の複合エンベロープは Alpha で大きく進んでいますが、曲用の本格的な
`.MUS`出力などは未保証です。音量の保持／自動変化（`f:count`／`f=count`）は
MGSC テキストと試聴用バイト列へ直列化されます。

音色ライブラリは**現状でも保存・読込はできます**。複合音色の保存形式17では音量の
「自動変化」と「詳細」も保存し、形式16は詳細オフ、形式15以前は自動変化オフとして読み込みます。
設定 INI やその他の保存形式については、各版の変更履歴を確認してください。
大切な音色は MGSC 定義テキスト（`.txt`）やクリップボードでも控えておくことを推奨します。
...
- **ライブラリ機能は未完成**です。保存できても、Alpha 中は仕様変更で互換性が崩れ得ます（Beta 以降で互換方針を開始）
