...
- **ライブラリ管理**: お気に入り、最近使用順、複製・名前変更・参照安全な削除、選択時試聴とA/B比較
...
- ライブラリ操作は右側パネルの `新規`／`読込`／`保存`／`別名保存`／`取消`／`複製`／`名前変更`／`削除`／`取込`／`書出` です。未保存編集を`取消`する場合は確認します
- `タグ管理`では独自タグを全SCC／OPLL／複合音色に対して名前変更・統合・削除できます（標準タグは対象外）
