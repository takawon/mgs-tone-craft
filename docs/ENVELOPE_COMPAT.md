...
- 発音前のトラック`@\`は`@e`内`\`やトラック`\`の生`FrequencyDelta`ではない。PSG／SCCは1/8周期換算＋13bit折り返し、OPLLは現在音から次半音までのF-number補間としてキーオン時に適用する。提示されたMGSDRV VGM境界値はPSG／SCCとOPLLで別の回帰列として保つ。
