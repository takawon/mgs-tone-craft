...
PSG/SCC: 符号反転して周期値へ加算。OPLL: F-Number へ加算したあと、値が `0xAC` 未満なら Block を下げて `0xAD` を足し、`0x160` 以上なら Block を上げて `0xAD` を引く（9bitクランプだけだと大きな `\` でオクターブがずれる）。
...
- 音量・音色・`y` 等はレーン別バイト列が残る。`\` は音量 `@e` ストリームへ載せて試聴する（別レーン待ちだと MGSDRV より1カウント早く周波数が動く）。
