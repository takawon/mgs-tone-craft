...
## 正式公開後の余興（優先度最低・初期Release対象外）

- [ ] 共有クリップ: 試聴PCM → `writeWavePcm` → 任意の外部`ffmpeg`で短いmp4。失敗時はWAV。
- [ ] MGSC `@v`／`@s`（および複合音色定義）を投稿用テキストとしてクリップボードへコピー。
- [ ] UIは画面キャプチャせず、静止画＋音声の直生成に留める。
- [ ] X APIへの自動投稿はさらに後段。まずは人が貼る書き出しまで。
